...
- 영속성: JPA(Hibernate) + PostgreSQL (로컬 dev 는 H2 PostgreSQL 호환 모드), 스키마는 Flyway
...
공유 인프라(`infra-postgres`/`infra-redis`/`kafka`, 네트워크 `infra-net`)와 `wms_master`/`wms_inventory`
DB 는 미리 존재해야 한다.
...
`infra/compose.yaml` 에 Postgres/Redis/Kafka. datasource/redis/kafka/jwt 는 **환경변수로 주입**
(`DB_*`/`REDIS_*`/`KAFKA_BOOTSTRAP_SERVERS`/`JWT_SECRET`)하며 기본값은 로컬 dev(localhost/H2).
...
값은 환경변수 주입(`${ENV:기본값}`), 시크릿 커밋 금지 — config-repo 는 **구조**만 담는다.
