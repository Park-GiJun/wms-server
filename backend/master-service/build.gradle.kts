plugins {
    // 품목·로케이션·거래처 마스터 + 신원(user 마스터). 헥사고날 + CQRS, DB-per-service → JPA.
    // 신원 마스터를 소유하므로 이 서비스가 JWT 를 발급한다(검증은 gateway). 발급 책임을 옮기려면
    // 아래 security-crypto/jjwt 런타임 의존성과 config-repo/master-service.yml 의 jwt 블록을 함께 이동.
...
dependencies {
    ...
    // JWT 발급(서명)은 master(신원 소유) 책임. shared 는 api 만 노출하므로 impl/jackson 을 직접 런타임에 둔다.
